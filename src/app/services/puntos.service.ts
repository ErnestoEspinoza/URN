...
export class FacultadesService {
  ...
  getFacultades() {
    return this.facultades = this.facultadesCollection.snapshotChanges().pipe(map(changes => {
      return changes.map(action => {
        const data = action.playload.doc.data() as Facultades;
        data.id = action.playload.doc.id;
        return data;
      });
    }));
  }
